...
_TECHNICAL_PAT = re.compile(
    ...
    r"|rate\s+limit|async|await|cors|ssl|tls|jwt|token|payload)\b", re.I
)
...
def detect_tone_profile(ticket: SupportTicket) -> ToneProfile:
    ...
    if tech_hits: signals.append(f"technical_terms:{tech_hits}")
    if biz_hits:  signals.append(f"business_terms:{biz_hits}")

    return ToneProfile(user_type=user_type, tone=tone,
                       complexity=complexity, profile_signals=signals)
