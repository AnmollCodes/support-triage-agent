...
_PATTERNS: List[Tuple[str, re.Pattern, str, str]] = [
    ...
    ("generic_card_16", re.compile(r"\b(?:\d{4}[\s\-]){3}\d{4}\b"), "[CARD-REDACTED]", "critical"),
    (
        "cvv",
    ...
    ("pan_card", re.compile(r"\b[A-Z]{5}[0-9]{4}[A-Z]\b"), "[PAN-REDACTED]", "high"),
    ("ssn_us", re.compile(r"\b\d{3}[-\s]?\d{2}[-\s]?\d{4}\b"), "[SSN-REDACTED]", "critical"),
    (
        "passport",
    ...
    ("phone_india", re.compile(r"\b(?:\+91[\s\-]?)?[6-9]\d{9}\b"), "[PHONE-REDACTED]", "medium"),
    (
        "phone_international",
